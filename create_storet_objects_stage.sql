...
create or replace package body create_storet_objects as

    procedure create_station_no_source is
    begin
    ...
    procedure create_result_no_source is
    begin
        ...
        execute immediate q'!insert /*+ append */ into result_no_source
                             ...
                             select /*+ full(r) use_hash(r) parallel(r, 4) full(s) use_hash(s) parallel(s, 4) */
                                    ...
                                    cast(trim(s.organization) || '-' || trim(r.activity_id) || nvl2(trim(r.trip_id), '-' || trim(r.trip_id), null) ||
                                       nvl2(trim(r.replicate_number), '-' || trim(r.replicate_number), null) as varchar2(4000 char)) activity,
        ...
        commit;
    end create_result_no_source;

    procedure create_activity_no_source is
    begin
        dbms_output.put_line(systimestamp || ' creating activity_no_source...');
        execute immediate 'truncate table activity_no_source';
        execute immediate q'!insert /*+ append */ into activity_no_source
                                    (data_source_id,
                                     data_source,
                                     station_id, 
                                     site_id,
    ...
    begin
        ...
        create_station_no_source;
        create_result_no_source;
        create_activity_no_source;
