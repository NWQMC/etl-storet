		...
		execute immediate q'!insert /*+ append nologging */ into fa_regular_result
			...
			from
				...
				left join wqp_nemi_epa_crosswalk nemi
					on trim(fa_regular_result_no_source.analytical_procedure_source) = nemi.analytical_procedure_source
					and trim(fa_regular_result_no_source.analytical_procedure_id) = nemi.analytical_procedure_id!';
		commit;
		...
		execute immediate q'!insert /*+ append nologging */ into fa_regular_result
			...
			from
				...
				left join di_activity_medium
					on fk_act_medium = di_activity_medium.pk_isn
				left join wqp_nemi_epa_crosswalk nemi
					on trim(storetw_fa_regular_result.analytical_procedure_source) = nemi.analytical_procedure_source
					and trim(storetw_fa_regular_result.analytical_procedure_id) = nemi.analytical_procedure_id!';
		commit;
		...
		loop
			...
			insert /*+ append nologging */ into biological_result_temp
				...
				select /*+ parallel (4) */
					...
					xml_helpers.biological_activity(
						xml_helpers.strip_bad(fa_biological_result.organization_id),
	...
	begin
		...
		execute immediate q'!insert /*+ append nologging */ into storet_sum
			...
			where
				...
				and length(trim(station.county_cd)) = 3
				and trim(station.state_cd) between '01' and '56'
				and result.fk_station = station.pk_isn
			group by
		...
		execute immediate 'insert /*+ append nologging */ into country
			...
			from
				(select /*+ parallel (4) */
					distinct country_cd code_value,
			...
			from
				...
				where exists
					...
					from
						fa_station
					where
						di_geo_county.pk_isn = fk_geo_county)
				order by
					country_code desc,
					fips_state_code,
					fips_county_code)!';
		...
		execute immediate 'insert /*+ append nologging */ into samplemedia
			select
				...
				rownum sort_order
			from
				(select /*+ parallel (4) */ distinct activity_medium as code_value
				from
					fa_regular_result
				where
		...
		execute immediate 'insert /*+ append nologging */ into sitetype
			...
			from
				(select /*+ parallel (4) */
					distinct station_group_type code_value
				from
					fa_station
				where
					station_group_type is not null
				order by
					1)';
