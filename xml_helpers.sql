...
as
	...
	function biological_activity(	organization_id				   fa_biological_result.organization_id%type,
									...
									md_smp_proc_prep_proc_name	   md_sample_proc.procedure_name%type,
									md_smp_proc_prep_proc_qual_typ md_sample_proc.procedure_qual_type%type,
									md_smp_proc_prep_description   md_sample_proc.description%type)
		return clob deterministic;
...
as
				...
				content(
					xmlelement("Activity",
						xmlelement("ActivityDescription",
							xmlelement("ActivityIdentifier", organization_id || '-' || activity_id || nvl2(trip_id, '-' || trip_id, null) || nvl2(replicate_number, '-' || replicate_number, null)),
						...
						xmlelement("SampleDescription",
							xmlelement("SampleCollectionMethod",
								...
								xmlelement("MethodName", md_smp_proc_proc_name),
								xmlelement("MethodQualifierTypeName", md_smp_proc_proc_qual_type),
								xmlelement("MethodDescriptionText", md_smp_proc_description)
							),
	...
	is
	begin
		return regexp_replace(p_string, '[^[:print:]]', ''); -- strip everything other than ASCII chars 32 to 126
	end strip_bad;
